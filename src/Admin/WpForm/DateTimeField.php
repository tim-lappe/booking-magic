...
class DateTimeField extends  FormFieldBase {
	...
	function OutputHtml() {
		...
		<tr>
			...
			<td>
                <?php if($this->endless_checkbox): ?>
                <div class="tlbm-endless-checkbox-container">
                    <label>
                        <input type="checkbox" <?php checked($this->value == PHP_INT_MAX) ?> name="<?php echo $this->name ?>_isEndless" class="regular-text endless-checkbox"> <?php echo __("endless", TLBM_TEXT_DOMAIN); ?>
                    </label>
                </div>
                <?php endif; ?>
                <div class="tlbm-select-datetime">
                    <div class="tlbm-date">
                        <p><strong><?php echo __("Date", TLBM_TEXT_DOMAIN); ?></strong></p><input id="<?php echo $this->name ?>" class="regular-text" type="date" value="<?php echo $datetime->format("Y-m-d") ?>" name="<?php echo $this->name ?>">
                    </div>
                    <div class="tlbm-time">
                        <p><strong><?php echo __("Time", TLBM_TEXT_DOMAIN); ?></strong></p>
                        <select name="<?php echo $this->name ?>_hours">
                            <?php for($i = 0; $i < 24; $i++): ?>
                                <option <?php echo intval($datetime->format('G')) == $i ? "selected='selected'" : "" ?> value="<?php echo $i ?>"><?php echo $i ?></option>
                            <?php endfor; ?>
                        </select>
                        &nbsp;<strong>:</strong>&nbsp;
                        <select name="<?php echo $this->name ?>_minutes">
                            <?php for($i = 0; $i <= 59; $i++): ?>
                                <option <?php echo intval($datetime->format('i')) == $i ? "selected='selected'" : "" ?> value="<?php echo $i ?>"><?php echo $i ?></option>
                            <?php endfor; ?>
                        </select>
                    </div>
                </div>
